...
from gads import get_gads_mcc_ids, get_gads_customer_ids, get_gads_campaigns, get_gads_data, process_gads_responses
from ga4 import get_ga4_data, get_ga4_account_ids, get_ga4_property_ids
from csv_data import get_csv_data, get_csv_columns, csv_get_date_range, csv_merge_data
    ...
    if "credentials" in session:
        ...
        if 'gads' in data:
            for key in data['gads']['query']:
                gads_responses.append(get_gads_data(data['gads']['mcc_id'], key, data['gads']['query'][key], data['from_date'], data['to_date']))        
            raw_data = process_gads_responses(gads_responses, data['gads']['metrics'])

        if 'ga4' in data:
            raw_data = get_ga4_data(data['ga4']['property'], data['from_date'], data['to_date'], data['ga4']['metrics'], raw_data)
        ...
        if raw_data:
            ...
            df=df[[data['target_event']] + [c for c in df if c not in [data['target_event']]]] #Puts target event as first column

            if 0 in df.values:
                warnings = "Some values have 0 in them which are from your data or where dates were missing and were filled in automatically"
            ...
            return render_template("report.html", summary=summary, chart=chart, report=report, raw_data=df.to_html(), warnings=warnings)
        else:
            return render_template("report.html", summary="No data in datasources!", chart="No data in datasources!", report="No data in datasources!", warnings="No data in datasources!")
    else:
        return redirect("/")
...
def upload_file():
    if request.method == 'POST':
        ...
        if not csv_date_column:
            ...
            csv_settings.append("'date' column not found. Add 'date' heading and use date format 'yyyy-mm-dd' in the values")
            ...
            if _validate_date(start_date) and _validate_date(end_date):
                ...
                csv_settings.append("error")
                csv_settings.append("'date' column found but some data does not match 'yyyy-mm-dd' format")
