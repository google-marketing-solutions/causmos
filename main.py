...
def root():
  ...
  return render_template('index.html', authed=authed, auth_analytics=auth_analytics, auth_adwords=auth_adwords, auth_sheets=auth_sheets, auth_slides=auth_slides, project_id=PROJECT_ID)
...
def getCsvSettings(csv_data: dict, format: str) -> dict:
  try:
    ...
    csv_settings.append('error')
    csv_settings.append('Error in format of the CSV file')
    return csv_settings
