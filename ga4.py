...
def get_ga4_account_ids() -> list:
    ...
    for account in response['accounts']:
        account_id_list.append([((account['name']).split("/"))[1], account['displayName']])
    
    return account_id_list

def get_ga4_property_ids(account_id) -> list:
    analytics_admin = get_analytics_admin_client()
    response = analytics_admin.properties().list(filter="parent:accounts/"+account_id).execute()
    property_id_list = []
    for properties in response['properties']:
        property_id_list.append([((properties['name']).split("/"))[1], properties['displayName']])
...
def get_ga4_data(property_id, date_from, date_to, items, raw_data):
    ...
    response = analytics.properties().batchRunReports(property="properties/"+property_id, body=request).execute()
    headers = []
    for report in response.get('reports', []):
        ...
        for row in report.get('rows', []):
            for date in row.get('dimensionValues'):
                date = datetime.strptime(date.get('value'), '%Y%m%d').strftime('%Y-%m-%d')
